...
There is only one standardized version of this base specifcation - no versioning.
...
- [Arrays](#Arrays): Arrays are comma-seperated collection of strings, enclosed by square brackets.
...
- [Files](#Files): XSON has stanardization for file extensions, line endings, encodings, and its internet media type that should be used.

## Keys/Values
[Key/Value pairs](https://en.wikipedia.org/wiki/Name%E2%80%93value_pair) are the bread and butter of XSON. 

A **key** is a string representing the name for the corrersponding value.

Keys in XSON are seperated from their values by the colon `:` character. Each key/value pair is seperated by a [newline](#L100). Keys (and all strings for that matter) should not have quotes, and can have any amount of whitespace:
```java
key: value
my key: my value
```
...
The first colon character read from left to right is used to seperate the key from the value:
```xson
my : character: 0x3A 
```
...
Spaces within the key names are allowed and perserved; Trailing and leading whitespace around the key and value are ignored. I.e., these are all equivalent:
```java
...
my long key: my long value
```
`my long key` evaluates to `my long value` in all these examples. It is of course recommended however to use as little whitespace as you need to perseve readability, as with the first example in this section above. 

<br/>

It is also recommended to conform with the naming convention of the lanuage when naming keys; For example:
```java
...
## Arrays
An array is a comma-seperated collection of strings, enclosed by square brackets `[]`.
```java
my array: [
  value one,
  value two,
...
Extension sets are specified using the `extset` directive and the set's name. Like with extension directives, you can use multiple extensions set directives or have multiple set names in a comma-seperated list. 

For example, specifying the `json` extension set would automatically enable all extensions which are needed for JSON backwards compability:
```cpp
#extset json, yaml, ini //Enable all extensions needed for json, yaml, and ini backward compatinility
...
Unlike extensions, extension sets have multiple versions. An extension set version consists of just a major an minor version (e.g. `1.3` or `2.5`). New extensions can be added to an extension set or existing extensions in a set can be replaced with other extensions that have the same functionality. Note how you cannot remove exisitng functionality from an extension set; This is to preserve backwards-compatibility so that all newer versions of a set are compatible with all previous versions, including the very first version of the set (i.e. version 1.0). This also means if an XSON parser/library supports a newer version of an extension set than what you want to use/specify, you will not run into problems as the newer version is garaunteed to be compatible with the older version you are using.
...
Specifying an extension set version is done by just appending it to the set name in the directive, seperated by a hypen:
```
